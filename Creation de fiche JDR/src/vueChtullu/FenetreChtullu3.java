...
public class FenetreChtullu3 extends JFrame implements DataChtullu {
	...
	private static final Logger logger = LogManager.getLogger(FenetreChtullu3.class.getName());
	...
	public FenetreChtullu3(ArrayList<String> caractPerso, ArrayList<Boolean> competenceBool,
	...
	private void screen1() {
		...
		containerLabel.add(new JLabelP("Vous avez " + comp + " points de compétence personnelle à distribuer.", font));

		containerCompetence.setLayout(new GridLayout(65, 1));
		for (i = 0; i < competence.length; i++) {
			competenceBT[i] = new BoutonComp(competence[i], competenceBool.get(i),
					Integer.parseInt(competenceInit.get(i).toString())); // Integer.parseInt(caractPerso.get(8).toString())
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if ((comp - compT) > 0) {
					...
					jop1.showMessageDialog(null,
							"Il vous reste " + (comp - compT) + " point de comptétence à distribuer.", "Attention",
							JOptionPane.INFORMATION_MESSAGE);
				} else if ((comp - compT) < 0) {
					jop2 = new JOptionPane();
					jop2.showMessageDialog(null, "Vous utilisez " + (comp - compT) + " point de compétence en trop.",
					...
					initCompGeneral();
					logger.debug("Compétence perso : " + compGPerso.toString());
	...
	private class auto implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			logger.debug("Compétence perso auto: " + compGPerso.toString());
			FenetreChtullu4 suiv = new FenetreChtullu4(caractPerso, compGPerso);
			dispose();
		}
	}
	...
	private class reload implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			// recharge l'écran avec les données actualisé
			compT = 0;
			...
			containerLabel.add(new JLabel("Vous avez " + (comp - compT) + " points de compétence metier à distribuer."));
			container.add(containerLabel);
			repaint();
			revalidate();
		}
