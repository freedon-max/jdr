...
public class FenetreChtullu2 extends JFrame implements DataChtullu {
	...
	private static final Logger logger = LogManager.getLogger(FenetreChtullu2.class.getName());
	...
	private void screen1() {
		...
		containerLabel.add(new JLabelP("Vous avez " + comp + " point de compétence metier à distribuer.", font));
	...
	private class auto implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			logger.debug("Compétence métier auto: " + compMetierPerso.toString());
	...
	private class reload implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			// recharge l'ecran avec les données à jour
			compT = 0;
	...
	private class next implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			// pour passer a l'ecran suivant, message d'avertissement si il
			// reste des points ou si trop d'utilisés
			JOptionPane jop1, jop2;
			if ((comp - compT) > 0) {
				jop1 = new JOptionPane();
				jop1.showMessageDialog(null, "Il vous reste " + (comp - compT) + " points de comptétence à distribuer.",
						"Attention", JOptionPane.INFORMATION_MESSAGE);
			} else if ((comp - compT) < 0) {
				jop2 = new JOptionPane();
				jop2.showMessageDialog(null, "Vous utilisez " + (comp - compT) + " points de compétence en trop.",
						"Attention", JOptionPane.INFORMATION_MESSAGE);
			} else {
				for (int m = 0; m < competence.length; m++) {
					compMetierPerso.add(competenceBT[m].getComp());
				}
				logger.debug("Compétence métier : " + compMetierPerso.toString());
				FenetreChtullu3 suiv = new FenetreChtullu3(caractPerso, competenceBool, compMetierPerso);
				dispose();
			}
		}
	...
	private int[] initSpec() {
		// initialise les valeurs initiale des compétence esquiver (dex*2) et
		// langue natale(edu*5)
